...
def display_pdf_clean(file_path):
    """
    Converts PDF pages to high-resolution images and displays them in a
    clean, scrollable HTML container. This is the most compatible way
    to show documents across all modern browsers.
    """
    try:
        ...
        for page in doc:
            # Zoom 2.0x for crisp text rendering
            pix = page.get_pixmap(matrix=fitz.Matrix(2, 2))
            img_b64 = base64.b64encode(pix.tobytes("png")).decode('utf-8')

            # Appending each page as an image
            img_stack += f'<img src="data:image/png;base64,{img_b64}" \
            style="width:100%; margin-bottom:15px; border-radius:4px; \
            box-shadow: 0px 4px 8px rgba(0,0,0,0.1);">'

        # The Scrollable "Vault" Container
        viewer_html = f"""
            <div style="
                height: 550px;
                overflow-y: scroll;
...
def render_smart_extraction(item):
    """Renders structured AI extraction results directly from the backend store."""
    ...
    # Pure data mapping from backend (no frontend calculations)
...
def show_demo_page():
    ...
    **Intelligence Checks:**
    ...
    - ✅ **Mathematical Consistency**: Validated via backend logic.
    """)
